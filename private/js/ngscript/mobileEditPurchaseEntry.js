// Edit Purchase Entry
app.controller('mobileEditPurchaseEntry', function($scope, $http) {
	$scope.partyInfo = {
		partyCode: '',
	...
	$scope.$watch('vno', function(voucherNo) {
        ...
        $scope.imeiExistenceCheck = function($index){

            var where = {
                table: 'stock',
                cond:{
                    'product_serial': $index,
                },
                select: ['product_serial', 'quantity']
            }
            $http({
    			method: 'POST',
    			url: url + 'result',
		...
		$http({
			...
			if(response.length > 0) {
				...
				angular.forEach(response, function(row, index){
					...
					response[index].imeiNO = row.product_serial ? row.product_serial : '';
					response[index].imeiNOStatus = row.product_serial ? true : false;
