...
class Dashboard extends Admin_controller{
    ...
    public function index() {
        ...
        $this->data['mdTransactionReceived'] = 0;
        $this->data['mdTransactionPaid'] = 0;


        $privilege = $this->data['privilege'];
        //showroom transaction
        if($privilege != 'super'){
            $mdTransactionWhere['godown_code']  = $this->data['branch'];
        }
        ...
        $this->data['mdTransactionReceived']    = get_sum('md_transactions','amount', $mdTransactionWhere);
        
        
        // MD Transaction Paid =>
        $mdTransactionWhere['type']         = 'Paid';
        $this->data['mdTransactionPaid']    = get_sum('md_transactions', 'amount',$mdTransactionWhere);
        ...
        $where = [];
        if($privilege == 'super'){
            $where = ['transaction_at' => $today, 'trash' => 0, 'transaction_by' => 'client'];
        }else{
        ...
        $this->data['client_collection'] = (!empty($todaClientTran) ? $todaClientTran : 0) +  (!empty($todaDueCollection) ? $todaDueCollection : 0);
        ...
        $where = [];
        if($privilege == 'super'){
            $where = array('transaction_at' => $today, 'trash' => 0, 'transaction_by' => 'supplier');
        }else{
