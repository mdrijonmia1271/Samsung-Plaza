...
class Retail_sale extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['allProducts'] = $this->action->readOrderby('stock', 'name', ['category !=' => 'mobile'], 'asc');
    ...
    private function sendSMS()
    {
        ...
        if ($_POST['current_balance'] == 'Due') {
            $content = "নামঃ " . filter($this->input->post('party_code')) . ", বিল নংঃ " . $this->data['voucher_no'] . ", বিল এমাউন্টঃ " . $this->input->post("grand_total") . " Tk, " . ", জমাঃ " . $this->input->post('paid') . " Tk, মোট বাকীঃ " . $this->input->post("current_balance") . " Tk,  তাংঃ " . $this->input->post('date') . " Sharif Iqbal Store.";
        } else {
            $content = "নামঃ " . filter($this->input->post('party_code')) . ", বিল নংঃ " . $this->data['voucher_no'] . ", বিল এমাউন্টঃ " . $this->input->post("grand_total") . " Tk, " . ", জমাঃ " . $this->input->post('paid') . " Tk, মোট বাকীঃ 0 Tk,  তাংঃ " . $this->input->post('date') . " Sharif Iqbal Electronics.";
        }
    ...
    // delete data for mobile
    public function mobile_delete($vno = null){

        $saleInfo = get_result('sapitems', ['voucher_no' => $vno, 'trash' => 0], ['product_code', 'quantity', 'godown_code','product_serial']);
